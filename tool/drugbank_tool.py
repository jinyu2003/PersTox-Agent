#!/usr/bin/env python3
"""DrugBank local query tool for PersTox-Agent.
...
Two phases:
    ...
    conda run -n perstox python tool/drugbank_tool.py build
    # 2. query (O(1) seeks; no XML, no full-file scan)
    conda run -n perstox python tool/drugbank_tool.py info DB00006
    conda run -n perstox python tool/drugbank_tool.py name "warfarin"
    conda run -n perstox python tool/drugbank_tool.py interactions DB00682
    conda run -n perstox python tool/drugbank_tool.py targets DB00682
